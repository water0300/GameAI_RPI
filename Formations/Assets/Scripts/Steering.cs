...
//         if(agent.Path != null){
//             target.position = agent.Path.nodes[currNode].transform.position;
//             if(Vector2.Distance(agent.transform.position, target.position) <= 2f){
//                 currNode++;
...
//             target.orientationDeg = Mathf.Atan2(direction.x, -direction.y) * Mathf.Rad2Deg; //trial and error
//         }
